...
export default function StorefrontPage() {
  ...
  const storeId = params.storeId as string // e.g., "9eefe5f4-..."
  ...
  // Fetch Store Data & Products Concurrently
  const fetchStoreData = useCallback(async () => {
    setLoading(true)
    try {
      // Run both API calls at the same time for performance
      const [storeRes, productsRes] = await Promise.all([
        fetch(`/api/sellers/${storeId}`),
        fetch(`/api/marketplace?sellerId=${storeId}&limit=20`) // Reusing marketplace endpoint!
      ])
