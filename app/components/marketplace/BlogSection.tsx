...
const fallbackBlogImages = [
    '/img/menu/home2-blog-banner.jpg',
    '/img/menu/home5-blog.jpg',
    '/img/menu/home3-blog.jpg',
];

function BlogCard({ post, index }: { post: BlogPost; index: number }) {
    const [imgSrc, setImgSrc] = useState(post.image || fallbackBlogImages[index % fallbackBlogImages.length]);

    return (
        <div className="bg-white group rounded-none border border-gray-200 hover:border-[#222222] transition-all duration-300 overflow-hidden flex flex-col">
            <Link href={`/blog/${post.slug}`} className="block relative h-44 sm:h-48 overflow-hidden bg-[#111111]">
                <Image
                    fill
                    src={imgSrc}
                    alt={post.title}
                    ...
                    onError={() => setImgSrc(fallbackBlogImages[index % fallbackBlogImages.length])}
                />
                <div className="absolute top-3 left-3 bg-[#222222] text-white px-2.5 py-1 text-[9px] font-bold uppercase tracking-widest rounded-none shadow-sm">
                    {post.date}
                </div>
            </Link>
...
export default function BlogSection({ posts }: { posts: BlogPost[] }) {
    return (
        <section className="py-12 bg-[#f9f9f9] font-sans border-t border-gray-100">
